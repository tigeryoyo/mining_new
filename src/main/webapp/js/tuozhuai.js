...
var data12 = {
	"status" : "OK",
	"result" : [ "属性", "标题", "链接", "来源/发布人", "发布时间", "网站", "频道", "点击数", "回复数", "类型", "记者/作者", "内容长度", "分词", "摘要/内容", "发布日期" ]
}

function up_del() {
	$(".btn_up_del01").unbind('click').click(function() {
		...
		var time = $(this).parent("li").children("select.select03").val();
		var sourceType = $(this).parent("li").children("select.select04").val();
		upFile(fileArray[parseInt(arrary)], urlIndex, titleIndex, time, sourceType);
		localRefresh();
